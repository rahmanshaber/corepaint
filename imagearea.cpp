...
void ImageArea::open( const QString &filePath )
{
    ...
    if ( mImage->load( filePath ) ) {
        *mImage = mImage->convertToFormat( QImage::Format_ARGB32_Premultiplied );
        ...
        CPrime::InfoFunc::messageEngine( tr( "Can't open file \"%1\"." ).arg( filePath ), CPrime::MessageType::Warning, this );
    }
}
...
bool ImageArea::save()
{
    ...
    if ( !mImage->save( mFilePath ) ) {
        // Function from LibCPrime
        CPrime::InfoFunc::messageEngine( tr( "Can't save file \"%1\"." ).arg( mFilePath ), CPrime::MessageType::Warning, this );
        return false;
    }
...
bool ImageArea::saveAs()
{
    ...
    if ( !filePath.isEmpty() ) {
        ...
        if ( mImage->save( filePath, extension.toLatin1().data() ) ) {
            ...
            result = false;
            // Function from LibCPrime
            CPrime::InfoFunc::messageEngine( tr( "Can't save file \"%1\"." ).arg( filePath ), CPrime ::MessageType::Warning, this );
