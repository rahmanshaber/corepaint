    ...
    mPrevInstrumentSetted( false )
{
    ...
    setStyleSheet( CPrime::ThemeFunc::getStyleSheetFileContent(
                       CPrime::StyleTypeName::CorePaintStyle ) );
...
void corepaint::initializeNewTab( const bool &isOpen, const QString &filePath )
{
    if ( ui->paintTabs->count() < 4 ) {
        ...
        if ( !imageArea->getFileName().isNull() ) {
            ...
            QString color( CPrime::ThemeFunc::getStyleSheet()->value( "@color06" ).toString() );
            ...
            connect( imageArea, SIGNAL( sendCursorPos( QPoint ) ), this,
                     SLOT( setNewPosToPosLabel( QPoint ) ) );
        ...
        if ( !fileName.isEmpty() ) {
            ...
            CPrime::InfoFunc::messageEngine( "File Opened Successfully.", CPrime::MessageType::Info, this );
        } else {
            // Function from LibCPrime
            CPrime::InfoFunc::messageEngine( "File not Opened Successfully.", CPrime::MessageType::Info, this );
        }
    }
    else {
        // Function from LibCPrime
        CPrime::InfoFunc::messageEngine( "Reached page limit.", CPrime::MessageType::Warning, this );
    }
}
...
void corepaint::initializeMainMenu()
{
    ...
    connect( mSColorChooser, SIGNAL( sendColor( QColor ) ), this,
             SLOT( secondaryColorChanged( QColor ) ) );
...
void corepaint::on_save_clicked()
{
    if ( getCurrentImageArea()->save() ) {
        ...
        // Function from LibCPrime
        CPrime::InfoFunc::messageEngine( "File not Saved", CPrime::MessageType::Info, this );
    }
}
...
void corepaint::sendFiles( const QStringList &paths )
{
    if ( paths.count() ) {
        foreach ( QString str, paths ) {
            initializeNewTab( true, CPrime::ValidityFunc::checkIsValidFile( str ) );
        }
    }
}
